...
```bash
conda create -n scala_multiply python=3.9
conda activate scala_multiply
```
...
```
This command will create your personal repository under `/Repos/<username>/scala_multiply`.
...
- To trigger the release pipeline, get the current version from the `scala_multiply/__init__.py` file and tag the current code version:
```
